...
knitr::include_graphics("R_vs_excel_friendly.jpg")
...
- .qmd files are a newer format that extends Rmd.
...
- To create a variable you use the assignment operator `<-` or `=`. `<-` is the preferred way.
...
- You can tell something is a function because it will be a word followed by parenthesis. The word is the name of the function and the stuff inside the parenthesis are the arguments.
...
`this_is_a_fake_function(argument_1 = 1, argument_2 = 2, show_plot = TRUE)`
    - `this_is_a_fake_function` is the name of the function
    - `argument_1`, `argument_2`, and `show_plot` are the arguments
    - `1`, `2`, and `TRUE` are the values of the arguments

`this_is_also_a_fake_function(1, 2, TRUE)`
...
- Usage: shows how to use the function. You can also see defaults for arguments here.
...
### plot()
Visualize data.
This uses plot() to create scatter plots. The first call creates a basic scatter plot, while the second converts the 'cyl' variable to a factor for categorical visualization.
...
Here, png() opens a device to save the plot as an image file and dev.off() closes the device after plotting.
